...
function validateAndUpdateGameRoom(room, newState, playerTeam) {
  ...
  for (const newChar of newChars) {
    ...
    if (newChar.hp > oldChar.hp) {
      // Allow healing only if this character was healed by an ally adjacent to oldChar and healer has not healed yet
      const healer = oldChars.find(c =>
        c.team === newChar.team &&
        !c.hasHealed &&
        areAdjacent(c, oldChar)
      );

      const healedAmount = newChar.hp - oldChar.hp;

      if (!healer || healedAmount > healer.healAmount || newChar.hp > newChar.maxHp) {
        console.log('Validation failed: Invalid healing');
        return false;
      }
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('endTurn', () => {
    ...
    io.to(playerRoomId).emit('gameUpdate', room.gameState);
  });
  ...
  socket.on('updateGame', (newState) => {
    ...
    if (!playerRoomId) {
      ...
      socket.emit('invalidUpdate', 'You are not in a room.');
      return;
    }
    ...
    if (!valid) {
      socket.emit('invalidUpdate', 'Invalid game state update.');
      return;
    }
  ...
  socket.on('joinGame', () => {
    console.log('User requested to join game:', socket.id);
    if (waitingQueue.length > 0) {
      const opponentSocketId = waitingQueue.shift();
      const roomId = `room-${roomCounter++}`;
      const teamAssignment = { [socket.id]: 'B', [opponentSocketId]: 'A' };
      ...
      io.to(roomId).emit('gameStart', {
        roomId,
        team: teamAssignment[socket.id],
        gameState: rooms[roomId].gameState,
      });

      console.log(`Started game in ${roomId} between ${socket.id} (B) and ${opponentSocketId} (A)`);
    } else {
  ...
  socket.on('disconnect', () => {
    ...
    if (playerRoomId) {
      ...
      if (remainingPlayers.length === 0) {
        ...
        console.log(`Room ${playerRoomId} deleted due to no players`);
      } else {
        io.to(playerRoomId).emit('opponentLeft');
      }
    }
